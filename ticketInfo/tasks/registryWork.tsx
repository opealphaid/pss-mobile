...
import ModalRegistryWork from "@/components/layouts/modalRegistryWork";
...
export default function RegistryWorkTab({ tareaId, ticketId }) {
  ...
  const fetchRegistros = async () => {
    try {
      const res = await fetch(`${PATH_URL_BACKEND}/registro-trabajo/tareas/${tareaId}`);
      if (!res.ok) throw new Error("Error al obtener registros");
      const data = await res.json();

      const aprobados = data.filter((r: any) => r.estadoRegistro === "APROBADO");
      const autorizados = data.filter((r: any) => r.estadoRegistro === "AUTORIZADO");
      ...
      setRegistros(aprobados);
      setRegistrosAutorizados(autorizados);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      <div className="overflow-x-auto">
        <div className="flex justify-end mb-3 p-4">
          <button
        ...
        {(registros.length > 0 || registrosAutorizados.length > 0) && (
          <div className="mb-8">
            <h3 className="text-lg font-semibold mb-4 ml-2 text-gray-800 p-4">
        ...
        {(registrosPendientes.length > 0 || registrosRechazados.length > 0) && (
          <div className="mb-8">
            <h3 className="text-lg font-semibold mb-4 ml-2 text-gray-800 p-4">Solicitudes pendientes y rechazadas</h3>
            <table className="min-w-full divide-y divide-gray-200">
        ...
        {registros.length === 0 && registrosAutorizados.length === 0 && registrosPendientes.length === 0 && registrosRechazados.length === 0 && (
