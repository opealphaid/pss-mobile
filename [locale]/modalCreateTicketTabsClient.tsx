...
import ModalCreateTicketClientFoi from "@/components/FOI/modalCreateTicketClientFoi";
import { FaTimes } from "react-icons/fa";

interface ModalCreateTicketTabsClientProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: () => void;
}

export default function ModalCreateTicketTabsClient({
    isOpen,
    onClose,
    onSubmit
}: ModalCreateTicketTabsClientProps) {
                    ...
                    {showTabs && (
                        <div className="grid grid-cols-2 gap-2 w-1/2 bg-gray-100 rounded-lg p-1">
                            <button
                                ...
                                className={`rounded-md py-2 ${activeTab === "soporte" ? "bg-blue-500 text-white" : ""}`}
                            >
                                Ticket Soporte
                            </button>
                            <button
